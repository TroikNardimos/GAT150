...
{
    return (m_audioSource.get()->IsPlaying());
}
void AudioComponent::Stop()
{
    if (m_audioSource) m_audioSource.get()->Stop();
}

void AudioComponent::Pause(bool pause)
{
    if(m_audioSource) m_audioSource.get()->Pause(pause);
}
...
void AudioComponent::SetVolume(float volume)
{
    ...
    if (IsPlaying())
    {
        m_audioSource.get()->SetVolume(volume);
    }
}
...
void AudioComponent::SetPitch(float pitch)
{
    ...
    if (IsPlaying())
    {
        m_audioSource.get()->SetPitch(pitch);
    }
}
